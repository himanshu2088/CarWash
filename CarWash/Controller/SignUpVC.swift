...
class SignUpVC: UIViewController {
    ...
    @objc func createUser(_ sender: UIButton) {
        ...
        guard let name = nameTextField.text, let age = ageTextField.text, let email = emailTextField.text, let password = passwordTextField.text, let confirmPassword = confirmPasswordTextField.text, let gender = genderTextField.text, let address = addressTextField.text, let city = cityTextField.text, let phone = phoneTextField.text else { return }
        
        if name == "" || age == "" || email == "" || password == "" || confirmPassword == "" || gender == "" || address == "" || city == "" || phone == "" {
            ...
            Firestore.firestore().collection("workers").getDocuments { (snapshot, error) in
                ...
                for document in documents! {
                    ...
                    self.workersEmailArray.append(usedEmail)
                }
                ...
                Firestore.firestore().collection("users").getDocuments { (snap, error) in
                    ...
                    if self.emailArray.contains(email) || self.workersEmailArray.contains(email) {
                        self.spinner.stopAnimating()
    ...
    func saveData() {
        ...
        guard let name = nameTextField.text, let age = ageTextField.text, let email = emailTextField.text, let gender = genderTextField.text, let password = passwordTextField.text, let address = addressTextField.text, let city = cityTextField.text, let phone = phoneTextField.text else { return }
    ...
    override func viewDidLoad() {
        ...
        genderTextField.topAnchor.constraint(equalTo: self.nameTextField.bottomAnchor, constant: 10.0).isActive = true
        genderTextField.leadingAnchor.constraint(equalTo: self.view.centerXAnchor, constant: 5.0).isActive = true
        ...
        addressTextField.topAnchor.constraint(equalTo: self.cityTextField.bottomAnchor, constant: 10.0).isActive = true
        ...
        phoneTextField.topAnchor.constraint(equalTo: self.addressTextField.bottomAnchor, constant: 10.0).isActive = true
        phoneTextField.leadingAnchor.constraint(equalTo: self.scrollView.leadingAnchor, constant: 20.0).isActive = true
        ...
        emailTextField.topAnchor.constraint(equalTo: self.phoneTextField.bottomAnchor, constant: 10.0).isActive = true
